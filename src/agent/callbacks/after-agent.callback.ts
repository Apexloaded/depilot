...
import { WorkflowStatus, workflowStore } from '../../firebase/index.js';
import logger from '../../common/logger/index.js';

const VERDICT_PATTERN =
  /Executive Verdict:\**\s*(APPROVED|CAUTION_REQUIRED|FATAL_RISK|AWAITING_APPROVAL|AWAITING_INPUT)/i;
...
export async function finalizeWorkflowStatus(
  ...
  try {
    ...
    const currentStatus = context.state.get(workflowKeys.Status) as
      | WorkflowStatus
      | undefined;
    if (
    ...
    if (!match) {
      logger.warn('[afterAgentCallback] no verdict found in final response', {
        workflowId,
      });
      return undefined;
    }
    ...
    logger.info('[afterAgentCallback] workflow marked COMPLETED', {
      workflowId,
      verdict,
    });
